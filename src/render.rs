...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    let query_line = if app.query.is_empty() {
        ...
        let max_len = query_area.width as usize;
        ...
        Line::from(vec![Span::styled(display, Style::default().fg(Color::DarkGray))])
    } else {
        Line::from(vec![Span::raw(&app.query)])
    };
    f.render_widget(query_line, query_area);

    // Cursor position (char-count offset — known limitation for wide chars)
    f.set_cursor_position((
        query_area.x + app.query.chars().count() as u16,
        query_area.y,
    ));
}
